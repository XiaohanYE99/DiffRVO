...
class NavigationEnvs():
    ...
    def projection(self, action):
        ...
        if self.use_kernel_loop == False:
            ...
            velocity_x = (6 * torch.sum((0.25 * alpha + 1) * phix * torch.exp(-F.relu(alpha * r - 0.01) - 0.01),
                                         dim=1)) * self.mask_x
            ...
            velocity_y = (6 * torch.sum((0.25 * alpha + 1) * phiy * torch.exp(-F.relu(alpha * r - 0.01) - 0.01),
                                         dim=1)) * self.mask_y
        else:
    ...
    def get_velocity(self, pos, velocity):
        ...
        return 4*torch.cat((vel_x /rr, vel_y /rr), 1)  # .squeeze(2)
