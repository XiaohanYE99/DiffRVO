...
class NavigationEnvs():
    def __init__(self, batch_size, sim, multisim,use_kernel_loop, use_sparse_FEM):
    ...
    def projection(self, action):
        ...
        t0 = time.time()
        k = action.size(0)
        x0 = action[:, ::5].unsqueeze(2).unsqueeze(3) * 1.4 - 0.2
        y0 = action[:, 1::5].unsqueeze(2).unsqueeze(3) * 1.4 - 0.2
        phix = (action[:, 2::5].unsqueeze(2).unsqueeze(3) - 0.5) * 0.2
        phiy = (action[:, 3::5].unsqueeze(2).unsqueeze(3) - 0.5) * 0.2
        alpha = (action[:, 4::5].unsqueeze(2).unsqueeze(3)) * 28.0 + 2
        alpha[alpha <= 1] = 1
        ...
        if self.use_kernel_loop == False:
            ...
            velocity_x = (6 * torch.sum((0.1 * alpha + 1) * phix * torch.exp(-alpha * r ),
                                        dim=1)) * self.mask_x
            ...
            velocity_y = (6 * torch.sum((0.1 * alpha + 1) * phiy * torch.exp(-alpha * r ),
                                        dim=1)) * self.mask_y
        else:
    ...
    def get_velocity(self, pos, velocity):
        ...
        vel_y = vel_y / energy
        '''
        rr = torch.sqrt(torch.square(vel_x) + torch.square(vel_y)+ 1e-6)#1E-4

        #rr = F.relu(rr / 4.0 - 1.0) + 1.0
        return 5*torch.cat((vel_x /rr, vel_y /rr), 1)  # .squeeze(2)
    ...
    def MBLoss(self, xNew, x):
        ...
        for i in range(xNew.size(0)):
            ...
            dist = distnew - distold
            dist[dist > 0] = dist[dist > 0] * 1.5#5
            loss += torch.sum(dist)

        return 1.0*loss/xNew.size(0)
        '''
        xNew=xNew/self.scale
