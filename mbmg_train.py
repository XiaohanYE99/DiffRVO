...
class PolicyNet(nn.Module):
    def __init__(self, env, state_dim, action_dim, has_continuous_action_space, action_std_init=0.2
        ...
        if has_continuous_action_space:
            ...
            '''
            self.actor = nn.Sequential(
                nn.Conv2d(4, 8, 5, 1, 0, bias=False), nn.MaxPool2d(2), nn.Tanh(),  # [50,50,8]
            ...
            for name, param in self.actor.named_parameters():
                if (len(param.size()) >= 2):
                    nn.init.kaiming_uniform_(param, a=3e-1)
            #self.actor = torch.load('task2model/model_1620.pth')
            self.lr = 1e-4
    ...
    def switch(self, v,state,target):
        ...
        return alpha*v+(1.0-alpha)*(-10*torch.cat((x/r,y/r),1))
    ...
    def multiimplement(self, s, target, training):
        ...
        for i in range(self.env.n_groups):
            ...
            for k in range(self.env.N):
                self.env.x0[k] = action[0][5 * k]
                self.env.y0[k] = action[0][5 * k + 1]
        ...
        s_f = torch.zeros_like(s)
        v_f = torch.zeros_like(vg)
        s_f[:, ::2] = s[:, :self.env.n_robots]
        s_f[:, 1::2] = s[:, self.env.n_robots:]
        v_f[:, ::2] = vg[:, :self.env.n_robots]
        v_f[:, 1::2] = vg[:, self.env.n_robots:]
        if training:
            xNew_f = self.MultiCFLayer(self.env, s_f, v_f)
        else:
            xNew_f = self.CFLayer(self.env, s_f, v_f)
...
if __name__ == '__main__':
    ...
    for i in range(iter):
        ...
        sumloss += loss
        if i % 10 == 0:
            torch.save(policy.actor, 'task2model/model_%d.pth' % i)
        if i % 40 == 0:
